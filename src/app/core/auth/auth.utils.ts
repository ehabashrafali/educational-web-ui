...
export class AuthUtils {
   ...
   * @param claimKey - Optional claim name (if omitted, returns all claims)
   */
  static getClaim(token: string, claimKey?: string): any {
    // Return if no token
    if (!token || token === "") {
      return null;
    }
    ...
    try {
      ...
      // Return all claims if no key specified
      if (!claimKey) {
        return decodedToken;
      }

      // Return the specific claim if exists
      return decodedToken.hasOwnProperty(claimKey)
        ? decodedToken[claimKey]
        : null;
    } catch (error) {
      console.error("Error decoding token claim:", error);
      return null;
    }
  }
}
